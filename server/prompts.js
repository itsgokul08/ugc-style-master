...
3. Subject + identity — the exact same person as the reference photo in all three panels: same face, bone structure, skin tone, eye color, and hair color/texture; adjust apparent age to match the specified age if it differs from the reference photo. Hairstyle — how it's worn and combed (loose, tied back, up in a bun, wet, tousled, neatly combed) — should match the outfit reference image if one is provided, since that image shows how the hair is actually styled with that look; otherwise match the reference photo's hairstyle exactly.
